...
class CreateSerchIssue extends StatefulWidget {
  ...
  static void open({
    required SpeakWithSerchController controller,
    SpeakWithSerch? message,
  }) {
    Get.bottomSheet(
      CreateSerchIssue(
        message: message,
...
class _CreateSerchIssueState extends State<CreateSerchIssue> {
    ...
    if(_messageController.text.isEmpty) {
      ...
      try {
        ...
        if(response.isOk) {
          ...
          widget.controller.homeController.loadSpeakWithSerchMessages();
  ...
  void markRead() async {
    try {
      if(_message != null) {
        var res = await _connect.patch(endpoint: "/company/speak_with_serch/${_message?.ticket}", body: {});
        ApiResponse response = ApiResponse.fromJson(res.data);
        if(response.isOk) {
          widget.controller.homeController.updateSpeakWithSerch(response);
        }
      }
    } on Exception catch (_) {}
  }
