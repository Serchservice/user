...
class CameraLayoutController extends GetxController {
  ...
  final AppConfiguration config = Get.find<AppConfiguration>();
  ...
  @override
  void onInit() {
    state.isChat.value = _args["mode"] != null && _args["mode"] == "chat";

    if(state.isChat.value) {
      state.sendTo.value = _args["to"] ?? "";
    }

    state.callbackUrl.value = _args["callback_url"] ?? "";
    state.sendToId.value = _args["to_id"] ?? "";
  ...
  void initCamera() async {
    int position = state.isFrontCamera.value
      ? 1
      : 0;
  ...
  Future<SelectedMedia?> stopVideoRecording(CameraController camController) async {
    try{
      ...
      SnackBars.top(
        message: e.description ?? "Couldn't capture. Try again",
        type: Snackbar.error
      );
      return null;
    }
  ...
  void stopRecording() async {
    ...
    if(result != null && !state.isFetching.value) {
      result.copyWith(
        duration: state.videoDuration.value,
        isCamera: true
      );
      disposeCameraResources();
