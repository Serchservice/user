...
class BookmarkSheet extends StatefulWidget {
  ...
  static void open({required Bookmark bookmark, required BookmarkController controller}) => Navigate.bottomSheet(
    sheet: BookmarkSheet(bookmark: bookmark, controller: controller),
...
class _BookmarkSheetState extends State<BookmarkSheet> {
  ...
  void unbookmark () async {
    if(unbookmarking) {
      ...
      var response = await _connect.delete(endpoint: "/bookmark/remove?id=${widget.bookmark.id}");
      setState(() {
        unbookmarking = false;
      });
      if(response.isOk) {
        widget.controller.fetchBookmarks();
        widget.controller.home.messaging.fetchChats(showLoader: false);
        Navigate.back();
          ...
          ...buttons.map((button) => NavigatorButton(
            ...
            onPressed: () {
              if(button.index == 0) {
                ...
                RouteNavigator.openChat(roommate: widget.bookmark.user, removeRoute: true);
              } else {
