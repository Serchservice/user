...
class GuestActiveTripView extends StatelessWidget {
  final TripResponse trip;
  const GuestActiveTripView({super.key, required this.trip});

  static void open(TripResponse trip) {
    Navigate.toPage(
      widget: GuestActiveTripView(trip: trip),
      route: "/guest/activity/trip/active?id=${trip.id}",
    );
  }
  ...
  Widget _buildMap(BuildContext context, GuestActiveTripViewController controller) {
    return Obx(() {
      ...
      if(showSharedMap) {
        return MapView(
          ...
          height: Get.height,
          subscriptionEndpoint: "/platform/location/${trip.id}/${Database.guest.id}"
        );
      } else if(showMap) {
        return MapView(
          isTop: true,
  ...
  Widget _buildBody(BuildContext context, GuestActiveTripViewController controller) {
    return Obx(() {
      TripResponse selected = controller.state.trip.value;
  ...
  Widget? _buildChild(BuildContext context, TripResponse trip, TimelineResponse timeline, GuestActiveTripViewController controller) {
  ...
  Widget? _buildSharedChild(
      ...
      GuestActiveTripViewController controller
      ) {
    if (timeline.isRequested && trip.showCancel) {
      return Column(
        children: [
