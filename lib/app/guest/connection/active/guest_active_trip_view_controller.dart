...
class GuestActiveTripViewController extends GetxController {
  final TripResponse trip;
  GuestActiveTripViewController({required this.trip});

  final state = GuestActiveTripViewState();
  ...
  void verifyAuth(String code) async {
    ...
    var response = await _connect.patch(
      endpoint: "/trip/auth",
      body: {"code": state.authToken.value, "trip": trip.id, "guest": Database.guest.id}
    );
  ...
  void grantAccess() async {
    ...
    ApiResponse response = await _connect.patch(endpoint: "/trip/shared/access?id=${trip.id}&guest=${Database.guest.id}");
  ...
  void end() async {
    ...
    ApiResponse response = await _connect.patch(
      endpoint: "/trip/end",
      body: {"trip": trip.id, "guest": Database.guest.id, "link_id": Database.preference.active}
    );
