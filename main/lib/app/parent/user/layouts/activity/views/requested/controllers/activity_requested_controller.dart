...
class ActivityRequestedController extends GetxController {
  ActivityRequestedController();
  static ActivityRequestedController get data => Get.find<ActivityRequestedController>();

  final ActivityRequestedState state = ActivityRequestedState();

  final ConnectService _connect = Connect(showError: false, useToken: Database.isUserActive);
  final Socket _scheduleSocket = Socket();
  final Socket _tripSocket = Socket.instance;
  final Socket _tripListSocket = Socket.instance;
  ...
  Future<void> _fetchTrips(int pageKey) async {
    ...
    String url = Database.isUserActive
        ? "/trip/history/requested?$pagination"
        : "/trip/history/requested?guest=${Database.guest.id}&link=${Database.preference.active}&$pagination";
  ...
  Future<void> _fetchSchedules(int pageKey) async {
    if(Database.isUserActive) {
      var response = await _connect.get(endpoint: "/schedule/history/requested?page=$pageKey&size=$_pageSize");
  ...
  String filterRoute(String current) => "/activity/filter?tab=requested&current=$current";
  ...
  void onReady() {
    if(Database.isUserActive) {
      _scheduleSocket.initialize(
        ...
        endpoint: "/ws:serch",
        subscribeDestination: "/platform/schedule/pending/${Database.auth.id}"
      );
    }
    ...
    _tripSocket.initialize(
      ...
      endpoint: "/ws:serch",
      subscribeDestination: "/platform/$user/trip/requested"
    );

    _tripListSocket.initialize(
      callback: (frame) {
        if (frame.hasData) {
          List<dynamic> data = frame.data;
          List<TripResponse> list = data.map((d) => TripResponse.fromJson(d)).toList();
          updateTrips(list);
        }
      },
      endpoint: "/ws:serch",
      subscribeDestination: "/platform/$user/trip/requested/history"
    );
