...
class ActivityHistoryTripItemSheetController extends GetxController {
  final TripResponse trip;
  ActivityHistoryTripItemSheetController({required this.trip});

  final state = ActivityHistoryTripItemSheetState();
    ...
    ButtonView(
      header: state.trip.value.shared!.profile!.isBookmarked
        ? state.isBookmarkingInvited.value
          ? "Removing bookmark for ${state.trip.value.shared!.profile!.name}"
          : "Remove bookmark for ${state.trip.value.shared!.profile!.name}"
        : state.isBookmarkingInvited.value
    ...
    ButtonView(
      header: state.trip.value.provider!.isBookmarked
        ? state.isBookmarkingProvider.value
          ? "Removing bookmark for ${state.trip.value.provider!.name}"
          : "Remove bookmark for ${state.trip.value.provider!.name}"
        : state.isBookmarkingProvider.value
  ...
  void act(int index) {
    if(index == 0) {
      ...
      Navigate.back();
      ActivityHistoryTripView.open(state.trip.value);
    } else if(index == 2) {
  ...
  void rebookTripWithProvider() async {
    Loading.open();
    var response = await _connect.get(endpoint: "/trip/rebook?id=${state.trip.value.id}");
  ...
  void rebookTripWithInvited() async {
    Loading.open();
    var response = await _connect.get(endpoint: "/trip/rebook?id=${state.trip.value.id}&withInvited=true");
    if(response.isSuccessful) {
      TripResponse trip = TripResponse.fromJson(response.data);
      ActivityActiveController.data.addTrip(trip);
  ...
  void bookmarkInvited() async {
    if(state.isBookmarkingInvited.value) {
      ...
      if(state.trip.value.shared!.profile!.isBookmarked) {
        var response = await _connect.delete(endpoint: "/bookmark/remove?id=${state.trip.value.shared!.profile!.bookmark}");
        ...
        var response = await _connect.post(endpoint: "/bookmark/add", body: {"user": state.trip.value.shared!.profile!.id});
        state.isBookmarkingInvited.value = false;
  ...
  void bookmarkProvider() async {
    if(state.isBookmarkingProvider.value) {
      ...
      if(state.trip.value.provider!.isBookmarked) {
        var response = await _connect.delete(endpoint: "/bookmark/remove?id=${state.trip.value.provider!.bookmark}");
        ...
        var response = await _connect.post(endpoint: "/bookmark/add", body: {"user": state.trip.value.provider!.id});
  ...
  void shareInvited() async {
    if(state.isSharingInvited.value) {
      ...
      var response = await _connect.get(endpoint: "/guest/shared/create?id=${state.trip.value.id}&withInvited=true");

      state.isSharingInvited.value = false;
      if(response.isOk) {
        notify.success(message: "Link generated. Go to Centre -> Account -> Links to view your links.");
        SharedLinksController.data.sharedController.refresh();
      } else {
        notify.error(message: response.message);
      }
    }
  ...
  void shareProvider() async {
    if(state.isSharingProvider.value) {
      ...
      var response = await _connect.get(endpoint: "/guest/shared/create?id=${state.trip.value.id}");
