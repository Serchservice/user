...
class AccountTrustSheet extends StatelessWidget {
  final Color color;

  const AccountTrustSheet({super.key, required this.color});

  static void open(Color color) => Navigate.bottomSheet(
    sheet: AccountTrustSheet(color: color),
    route: "/home/account_trust_toolkit",
    isScrollable: true
  );
  ...
  Widget _buildRating(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _buildButton(context: context, onClick: () => Navigate.to(RatingLayout.route), text: "See your rating analysis")
      ],
    );
  }
  ...
  Widget _buildButton({required BuildContext context, required VoidCallback onClick, required String text}) {
    return LoadingButton(
      ...
      textColor: CommonColors.darkTheme,
      buttonColor: CommonUtility.lightenColor(CommonColors.hint, 40),
    );
  }
}