...
class MapViewController extends GetxController with GetTickerProviderStateMixin {
  ...
  @override
  void onInit() {
    state.canSubscribe.value = subscription.isNotEmpty;
  ...
  Future _moveMapCamera(LatLng target, {double zoom = 50, double bearing = 0}) async {
  ...
  void onReady() {
    if(state.canSubscribe.value) {
      _socket.initialize(
        ...
        subscribeDestination: subscription
      );
    }
    super.onReady();
  }
