...
class GalleryController extends GetxController {
  ...
  @override
  void onInit() {
    state.isVideo.value = bool.tryParse(params["is_video"] ?? "") ?? false;
    state.isChat.value = bool.tryParse(params["is_chat"] ?? "") ?? false;
    state.title.value = params["title"] ?? "";
    state.receiver.value = params["id"] ?? "";
      ...
      if(result != null) {
        if(result.files.first.path != null) {
          ...
          if(state.isChat.value) {
            ...
            Navigate.back(result: media.toJson());
          }
        }
      } else {
