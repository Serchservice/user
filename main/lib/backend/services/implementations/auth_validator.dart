...
class AuthValidator implements AuthValidatorService {
  ...
  @override
  void validateSession({required Function(String success) onSuccess, required Function(String error) onError}) async {
  ...
  void fetchAccounts({Function(List<Account> success)? onSuccess, Function(String error)? onError}) async {
    ...
    var response = await connect.get(endpoint: useToken ? "/account" : "/guest/shared/accounts?id=${Database.guest.id}");
